...
class Game:
    def __init__(self):
        ...
        # Sign entity
        sign = Sign('A sample text that is very big and will serve us to test the dialog event system. We have to '
                    'write a little bit more so that we get more than one line of text. '
                    'This text should appear cover more than one box'
                    )
        self.sign_ent_id = self.world.create_entity(sign)

        # Message box
        font = self.resource_manager.font('Px437_Portfolio_6x8.ttf', pygame.Color('black'), 8)
